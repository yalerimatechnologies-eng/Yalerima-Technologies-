...
const CONFIG = Object.freeze({
  ...
  STORAGE_NOTIFICATION_SOUND: "yalerima.dashboard.notificationSound",

  LOGIN: "login.html",

  COMING_SOON: "clients.html",
...
function captureDOM() {
  const ids = [
    ...
    "notificationList",
    "notificationEmpty",
    "notificationEnableBrowserButton",
    "profileAvatar",
    "profileInitials",
    "profileName",
    "profileRole",
...
/* ------------------------------------------------------------------ */
/* NOTIFICATION SYSTEM (additive — new client/contact message alerts) */
/* ------------------------------------------------------------------ */
...
function initNotificationPreferences() {
  ...
  try {
    stored = localStorage.getItem(
      CONFIG.STORAGE_NOTIFICATION_SOUND
    );
  } catch {}

  App.state.notificationSoundEnabled =
    stored === null ? true : stored === "1";

  updateNotificationSoundIcon();
}

function updateNotificationSoundIcon() {
  if (!App.dom.notificationSoundIcon) return;

  App.dom.notificationSoundIcon.className =
    App.state.notificationSoundEnabled
      ? "fa-solid fa-volume-high"
      : "fa-solid fa-volume-xmark";

  App.dom.notificationSoundToggle?.setAttribute(
    "aria-label",
    App.state.notificationSoundEnabled
      ? "Sound on. Click to mute notifications."
      : "Sound off. Click to enable notification sound."
  );
}
...
function toggleNotificationSound() {
  ...
  try {
    localStorage.setItem(
      CONFIG.STORAGE_NOTIFICATION_SOUND,
      App.state.notificationSoundEnabled ? "1" : "0"
    );
  } catch {}
...
function playNotificationSound() {
  if (!App.state.notificationSoundEnabled) return;
  ...
  try {
    ...
    console.warn("[Yalerima Notification Sound]", error);
  }
}
...
function requestBrowserNotificationPermission() {
  ...
  if (Notification.permission === "granted") {
    toast("Desktop notifications are already enabled.", "info");
    return;
  }
...
function maybeShowBrowserNotification(notification) {
  ...
  try {
    const browserNotification = new Notification(
      notification.title || "New Client Message",
      {
        body:
          notification.body ||
          "You have a new client message.",
        tag: `yalerima-notification-${notification.id}`
      }
    );
    ...
    console.warn("[Yalerima Browser Notification]", error);
  }
}

function renderNotificationBadge() {
  const count = App.state.notificationUnreadCount;

  if (App.dom.notificationBadge) {
    if (count > 0) {
      App.dom.notificationBadge.textContent =
        count > 99 ? "99+" : String(count);

      App.dom.notificationBadge.style.display = "flex";
    } else {
      App.dom.notificationBadge.style.display = "none";
    }
  }
}
...
function buildNotificationItem(notification) {
  ...
  avatar.textContent = initials(notification.title);

  const body = document.createElement("div");
  body.className = "notification-item-body";

  const name = document.createElement("strong");
  name.textContent = notification.title || "New client message";

  const subject = document.createElement("span");
  subject.className = "notification-item-subject";
  subject.textContent = notification.body || "New message received.";
...
function renderNotificationList() {
  const host = App.dom.notificationList;
  ...
  if (!App.state.notifications.length) {
    ...
    label.textContent = "No notifications yet";
  ...
  for (const notification of App.state.notifications) {
    host.appendChild(buildNotificationItem(notification));
  }
}
...
async function loadNotifications() {
  if (!App.state.authorized) return;
  ...
  try {
    const { data, error } = await App.supabase
      ...
      .limit(CONFIG.NOTIFICATION_PAGE_SIZE);

    if (error) throw error;

    App.state.notifications = data || [];
    ...
    console.warn("[Yalerima Notifications]", error);
  }
}
...
async function markNotificationRead(id) {
  ...
  const notification = App.state.notifications.find(
    (item) => item.id === id
  );
  ...
  App.state.notificationUnreadCount = Math.max(
    0,
    App.state.notificationUnreadCount - 1
  );
  ...
  try {
    ...
    console.warn("[Yalerima Notifications] mark read failed", error);
  }
}

async function markAllNotificationsRead() {
  const unreadIds = App.state.notifications
    .filter((item) => !item.is_read)
    .map((item) => item.id);

  if (!unreadIds.length) return;

  for (const item of App.state.notifications) {
    item.is_read = true;
  }

  App.state.notificationUnreadCount = 0;
...
function handleNotificationItemClick(notification) {
  ...
  if (notification.message_id) {
    openMessage(notification.message_id);
  }
}

function openNotificationPanel() {
  if (!App.dom.notificationPanel) return;

  App.dom.notificationPanel.classList.add("show");
  App.dom.notificationButton?.setAttribute("aria-expanded", "true");

  App.state.notificationPanelOpen = true;
...
function closeNotificationPanel() {
  ...
  App.dom.notificationPanel.classList.remove("show");
  App.dom.notificationButton?.setAttribute("aria-expanded", "false");

  App.state.notificationPanelOpen = false;
}
...
function handleIncomingNotification(record) {
  ...
  const exists = App.state.notifications.some(
  ...
  if (!exists) {
    App.state.notifications = [
      record,
      ...App.state.notifications
    ].slice(0, CONFIG.NOTIFICATION_PAGE_SIZE);
  ...
  maybeShowBrowserNotification(record);

  toast(
    `New message from ${record.title || "a client"}.`,
    "info"
  );
}
...
function bindNotifications() {
  ...
  App.dom.notificationMarkAllButton?.addEventListener(
    "click",
    (event) => {
      event.stopPropagation();
  ...
  App.dom.notificationSoundToggle?.addEventListener(
    "click",
    (event) => {
      event.stopPropagation();
  ...
  App.dom.notificationEnableBrowserButton?.addEventListener(
    "click",
    (event) => {
      event.stopPropagation();
  ...
  App.dom.notificationPanel?.addEventListener(
    "click",
    (event) => event.stopPropagation()
  );

  document.addEventListener("click", (event) => {
    if (!App.state.notificationPanelOpen) return;

    if (
      App.dom.notificationWrap &&
      !App.dom.notificationWrap.contains(event.target)
    ) {
      closeNotificationPanel();
    }
  });
